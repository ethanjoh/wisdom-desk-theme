...
### 변경 목적
- 카테고리/아카이브 목록 페이지에서 카드형 및 연도별 타임라인 보기 전환 토글 기능 및 세련된 연도별 타임라인 뷰 구현

### 주요 결정 사항
- `archive.php`:
  - 카테고리 타이틀 우측에 `[ 카드형 ]` / `[ 연도별 ]` 보기 전환 버튼 그룹(`.archive-view-toggle`) 추가
  - 현재 카테고리/태그의 전체 포스트를 연도별로 그룹화하여 렌더링하는 `.archive-timeline-view` 마크업 구현
  - 연도별 상단 퀵점프 칩 바, 연도 배지 및 글 개수, 날짜(MM.DD)+제목+댓글수+콤팩트 썸네일 구조 구성
- `js/theme.js`:
  - 카드형 그리드 ↔ 연도별 타임라인 뷰 실시간 전환 이벤트 처리
  - `localStorage` 기반 선택 뷰 모드 영구 유지(새로고침 및 페이지 이동 후에도 복원)
  - 연도별 상단 칩 클릭 시 해당 연도 그룹 즉시 필터링 동작 연동
- `style.css`:
  - 테마 헤더 버전을 `1.6.24`로 상향
  - `.archive-heading-wrap`: 카테고리 타이틀과 보기 전환 버튼의 감성적 flex 좌우 정렬
  - `.archive-view-toggle`: 모던한 캡슐형 토글 버튼 그룹 디자인
  - `.archive-timeline-view`: 세로 가이드선, 연도 배지, 호버 인터랙션, 모바일 반응형 최적화 스타일링
- `functions.php`:
  - `TISTORY_STYLE_VERSION` 상수를 `1.6.24`로 상향하여 브라우저 CSS/JS 캐시 즉시 갱신 반영
- `CHANGELOG.md`: 버전 1.6.24 변경 사항 기록
...
### 변경 목적
- 2.5D 픽셀아트 인트로 오프닝 기능 및 관련 에셋 원상 복구(롤백): 인트로 이전 상태(v1.6.18 데스크 일러스트 및 상단 태그라인 유지)로 복원

### 주요 결정 사항
- `front-page.php`: 인트로 오버레이 마크업, SKIP 버튼, 다시 보기 버튼 및 줌인 트랜지션 스크립트 제거
- `style.css`: 인트로 오버레이, 픽셀아트 렌더링, 조명 펄스 효과 및 관련 모바일 스타일 제거 후 버전 1.6.22로 상향
- `images/`: 인트로용 임시 이미지(`concept_intro_room_1.jpg`, `concept_intro_room_2.jpg`) 정리 삭제
- `functions.php`: `TISTORY_STYLE_VERSION` 상수를 `1.6.22`로 상향하여 브라우저 캐시 즉시 갱신 반영
- `CHANGELOG.md`: 버전 1.6.22 롤백 이력 기록

### 수정한 파일
- front-page.php
- style.css
- functions.php
- images/concept_intro_room_1.jpg (삭제)
- images/concept_intro_room_2.jpg (삭제)
- CHANGELOG.md

### 테스트 결과
- `php -l front-page.php` 및 `php -l functions.php` 문법 검사 통과
- 메인 데스크 일러스트 및 상단 명조체 이탤릭 태그라인 원상 복구 확인 완료
