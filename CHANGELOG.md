...
### 변경 목적
- 2.5D 픽셀아트 인트로 배경의 창문 크기를 적정 크기로 축소 조율: 벽면 전체를 덮던 과도한 통유리 대신, 책상 위쪽 벽면에 단정하게 어울리는 나무 프레임 창문으로 변경하여 아늑한 서재 무드 완성

### 주요 결정 사항
- `images/concept_intro_room_2.jpg`:
  - 모니터 뒤편 창문 크기를 적절히 줄이고 주변 책장/소품과 조화를 이루는 고품질 2.5D 픽셀아트 일러스트로 교체
- `style.css`:
  - 테마 헤더 버전을 `1.6.21`로 상향
- `functions.php`:
  - `TISTORY_STYLE_VERSION` 상수를 `1.6.21`로 상향하여 브라우저 캐시 즉시 갱신 반영
- `CHANGELOG.md`: 버전 1.6.21 변경 사항 기록
...
- 메인페이지 2.5D 인트로 화면에서 적정 크기 창문 및 서재 밸런스 확인 완료
...
### 변경 목적
- 2.5D 픽셀아트 인트로 배경을 현재 메인 일러스트 설정과 일치하도록 개선: 컴퓨터 책상(3개 모니터) 뒤편 벽면에 바깥 자연 풍경이 시원하게 보이는 대형 파노라마 통유리창 반영

### 주요 결정 사항
- `images/concept_intro_room_2.jpg`:
  - 모니터 뒤편에 커다란 통유리창과 아름다운 숲/하늘 풍경이 조화를 이루는 고품질 2.5D 픽셀아트 일러스트로 교체
- `style.css`:
  - 테마 헤더 버전을 `1.6.20`으로 상향
- `functions.php`:
  - `TISTORY_STYLE_VERSION` 상수를 `1.6.20`으로 상향하여 브라우저 캐시 즉시 갱신 반영
- `CHANGELOG.md`: 버전 1.6.20 변경 사항 기록
...
### 테스트 결과
- `php -l functions.php` 문법 검사 통과
- 메인페이지 2.5D 인트로 화면에서 대형 통유리창 및 서재 조명 렌더링 확인 완료
...
### 변경 목적
- 2.5D 픽셀아트 오프닝 인트로 구현: 캐릭터가 방 안으로 걸어 들어가 문 안쪽의 3개 모니터 데스크로 화면이 부드럽게 줌인(Zoom-in)되며 메인 일러스트로 연결되는 인터랙티브 오프닝 추가

### 주요 결정 사항
- `front-page.php`:
  - 2.5D 픽셀아트 인트로 오버레이(`.pixel-intro-overlay`, `concept_intro_room_2.jpg`) 마크업 배치
  - 건너뛰기("SKIP") 버튼 및 메인 일러스트 우측 상단 인트로 다시 보기("Replay") 버튼 추가
  - `sessionStorage` 기반으로 1회 방문 시 자동 1회만 재생되고, 새로고침/카테고리 이동 후 복귀 시에는 방해 없이 바로 메인 화면을 표시하도록 세션 제어 적용
  - 0.8초 후 문 안쪽 데스크(78% 48%)로 부드러운 줌인 후 1.6초 시점에 메인 일러스트로 크로스페이드 아웃되는 시네마틱 트랜지션 JS 스크립트 작성
- `style.css`:
  - 테마 헤더 버전을 `1.6.19`로 상향
  - `.pixel-intro-overlay`, `.intro-scene-wrapper`, `.intro-scene-img` 픽셀아트 크리스프 렌더링 및 줌인 트랜지션 스타일 정의
  - 문 안쪽 은은한 조명 펄스 애니메이션(`.intro-door-light`) 및 글래스모피즘 스킵/다시보기 버튼 스타일 정의
  - 모바일(640px 이하) 반응형 컨트롤 위치 및 크기 최적화
- `functions.php`:
  - `TISTORY_STYLE_VERSION` 상수를 `1.6.19`로 상향하여 브라우저 CSS/JS 캐시 즉시 갱신 반영
- `CHANGELOG.md`: 버전 1.6.19 변경 사항 기록
...
- 인트로 줌인 트랜지션, SKIP 버튼, 세션스토리지 1회 재생 및 다시보기(Replay) 버튼 동작 정합성 검증 완료
...
### 변경 목적
- 모바일 화면에서 카테고리/아카이브 글 목록의 좌우 여백이 과도하게 크게 나타나 카드 및 텍스트 영역이 좁아지는 현상 해결

### 주요 결정 사항
- `style.css`:
  - 테마 헤더 버전을 `1.6.13`으로 상향
  - 모바일 미디어 쿼리(`@media screen and (max-width: 1060px)`)에서 `body:has(.archive-modern-list) .main` 및 `.area-main`에 적용되던 중첩 패딩(32px + 20px)을 `0`으로 초기화하여 불필요한 이중 여백 제거
  - 카테고리/아카이브 목록(`.archive-modern-list`)이 화면 좌우 16px 마진(초소형 화면 480px 이하는 12px)으로 단일화되어 헤더(`padding: 0 16px`) 및 배너와 정렬선이 일치하고 카드 너비가 시원하게 넓어지도록 개선
- `functions.php`:
  - `TISTORY_STYLE_VERSION` 상수를 `1.6.13`으로 상향하여 브라우저 CSS 캐시 즉시 갱신 반영
- `CHANGELOG.md`: 버전 1.6.13 변경 사항 기록
...
### 테스트 결과
- `php -l functions.php` 문법 검사 통과
- 모바일 아카이브 레이아웃 여백 초기화 및 미디어 쿼리 CSS 정합성 검증 완료
...
### 변경 목적
- 모바일 햄버거 메뉴(사이드바 드로어)에서 상위 카테고리 옆의 글 개수 카운트가 하위 카테고리의 모든 글 개수를 합산하여 표시되도록 개선

### 주요 결정 사항
- `functions.php`:
  - `tistory_style_get_category_total_post_count()` 헬퍼 함수 구현: 상위 카테고리에 직접 등록된 글 수뿐만 아니라 모든 하위(자손) 카테고리에 속한 글 수까지 재귀적으로 합산하여 반환
  - `tistory_style_category_sidebar()` 함수에서 상위 카테고리 렌더링 시 총합 글 수를 적용하여 직속 글이 없더라도 하위 글이 있으면 정확한 총 글 수(예: 여행 (8), 북리뷰 (13))가 표시되도록 개선
  - `TISTORY_STYLE_VERSION` 상수를 `1.6.9`로 상향
- `style.css`:
  - 테마 헤더 버전을 `1.6.9`로 상향
- `CHANGELOG.md`: 버전 1.6.9 변경 사항 기록
...
### 변경 목적
- 1230px 노트북 및 와이드 화면에서 테마 콘텐츠(헤더, 본문 데스크 일러스트, 푸터 등)가 화면 좌우 끝에 바짝 밀착되어 답답해 보이는 현상 개선 및 적절한 시각적 안전 여백 확보

### 주요 결정 사항
- `style.css`:
  - 테마 헤더 버전을 `1.6.8`로 상향
  - 주요 레이아웃 컨테이너(`.header .inner-header`, `.main`, `#footer .inner-footer`, `.inner-bottom-widget`) 및 프론트페이지/홈 변형 컨테이너에 반응형 좌우 패딩(`padding: 0 32px; box-sizing: border-box;`) 적용
  - 1230px 노트북 화면에서 좌우 32px(총 64px)의 여백이 생겨 테두리에 붙지 않고 안정적인 비율로 콘텐츠가 표시되도록 구성
  - 1480px 이상의 대형 모니터 및 1060px 이하 모바일 환경의 기존 레이아웃에는 영향을 주지 않도록 기존 반응형 규칙 보존
- `functions.php`:
  - `TISTORY_STYLE_VERSION` 상수를 `1.6.8`로 상향하여 브라우저 CSS 캐시 즉시 갱신 반영
- `CHANGELOG.md`: 버전 1.6.8 변경 사항 기록
...
### 테스트 결과
- `php -l functions.php` 문법 검사 통과
- CSS 구문 검사 및 컨테이너별 패딩/box-sizing 정합성 검증 완료
...
### 변경 목적
- 서브 카테고리(아카이브) 및 검색 결과 하단의 페이징(Pagination) UI를 모던 라운드 카드 버튼 스타일로 개선

### 주요 결정 사항
- `style.css`:
  - 테마 헤더 버전을 `1.6.7`로 상향
  - `.area-paging`, `.archive-paging` 컨테이너 중앙 정렬 및 상하 충분한 여백 확보
  - 접근성 텍스트(`.screen-reader-text`) 클립 숨김 처리
  - 워드프레스 페이지네이션 요소(`.navigation.pagination`, `.nav-links`, `.page-numbers`) 전용 스타일 구현:
    - 기본 버튼: 38px 높이, 라운드 8px, 은은한 테두리(`border: 1px solid #e2e8f0`) 및 부드러운 그림자
    - 호버 효과: 위로 1px 살짝 떠오르는 애니메이션, 테마 오렌지-레드 컬러 강조 및 그림자 부각
    - 활성 페이지(`.current`): 테마 포인트 컬러(`background: #f05a3c`), 화이트 볼드 폰트, 오렌지 글로우 그림자 적용
    - 이전/다음 버튼: 좌우 패딩을 주어 정갈한 내비게이션 버튼 완성
    - 말줄임표(`.dots`): 테두리 없는 자연스러운 텍스트 처리
  - 모바일(`max-width: 767px`) 반응형 페이징 크기(34px) 및 간격(6px) 터치 최적화
- `functions.php`:
  - `TISTORY_STYLE_VERSION` 상수를 `1.6.7`로 상향하여 브라우저 CSS 캐시 즉시 갱신 반영
- `CHANGELOG.md`: 버전 1.6.7 변경 사항 기록
...
### 테스트 결과
- `php -l functions.php` 문법 검사 통과
- CSS 구문 검사 및 데스크톱/모바일 반응형 페이징 스타일 적용 검증 완료
...
### 변경 목적
- 4열 그리드 레이아웃의 서브 카테고리/아카이브 글 목록에서 글이 10개만 표시되어 마지막 행의 3, 4열이 비어 보이는 현상 해결 (3행 × 4열 = 12개 글 표시)

### 주요 결정 사항
- `functions.php`:
  - 테마 버전 상수(`TISTORY_STYLE_VERSION`)를 `1.6.6`으로 상향
  - `pre_get_posts` 액션 훅(`tistory_style_archive_posts_per_page`)을 등록하여 아카이브(카테고리/서브카테고리/태그 등) 페이지의 메인 쿼리 `posts_per_page`를 12로 설정
  - 4열 그리드 구조에 맞추어 3행을 빈칸 없이 꽉 채우도록 구성
- `style.css`:
  - 테마 헤더 버전을 `1.6.6`으로 상향
- `CHANGELOG.md`: 버전 1.6.6 변경 사항 기록
...
### 테스트 결과
- `php -l functions.php` 문법 검사 통과
- 아카이브 메인 쿼리 `pre_get_posts` 적용 로직 검증 완료
...
### 변경 목적
- PC 화면에서 서브카테고리(카테고리/아카이브) 목록에 보여주는 글 카드들을 1행 4열 그리드 배치가 되도록 수정

### 주요 결정 사항
- `style.css`:
  - 테마 헤더 버전을 `1.6.4`로 상향
  - PC 환경(기본 데스크톱)의 `.archive-card-grid` 레이아웃을 기존 3열(`repeat(3, minmax(0, 1fr))`)에서 4열(`repeat(4, minmax(0, 1fr))`)로 변경
  - 4열 카드 배치에 맞추어 카드 간격(`gap`)을 `20px 16px`로 최적화하고, 카드 제목(`.title`)의 폰트 크기를 `14px`로 미세 조정하여 가독성 개선
  - 태블릿(`max-width: 1060px`, 2열) 및 모바일(`max-width: 767px`, 1열 가로 카드) 반응형 레이아웃은 기존대로 온전히 유지
- `functions.php`:
  - `TISTORY_STYLE_VERSION` 상수를 `1.6.4`로 상향하여 브라우저 CSS 캐시 즉시 갱신 반영
- `CHANGELOG.md`: 버전 1.6.4 변경 사항 기록
...
- CSS 구문 및 PC(4열), 태블릿(2열), 모바일(1열) 반응형 브레이크포인트 유효성 검증 완료
